 ...
 */
class MarkerPlugin(private val mapplsMap: MapplsMap, val mapView: MapView) : MapView.OnDidFinishLoadingStyleListener, MapplsMap.OnMapClickListener {
            ...
            override fun onTouch(view: View?, motionEvent: MotionEvent?): Boolean {
                if(isDraggable) {
                    if(motionEvent!!.action == MotionEvent.ACTION_DOWN) {
                        ...
                        if(isMarkerPosition) {
                            val position: LatLng = mapplsMap.projection.fromScreenLocation(PointF(motionEvent.x, motionEvent.y))
    ...
    private fun isMarkerPosition(pointF: PointF) : Boolean{
        val features: List<Feature> = mapplsMap.queryRenderedFeatures(pointF, LAYER_ID)
    ...
    fun startTransition(latLngStart: LatLng, latLngEnd: LatLng) {
        ...
        valueAnimatorTransition.addUpdateListener {
            if(!isRemoveCallback) {
                ...
                if (!mapplsMap.projection.visibleRegion.latLngBounds.contains(latLng)) {
                    mapplsMap.animateCamera(CameraUpdateFactory.newLatLngZoom(latLng, 16.0))
                }
    ...
    private class GenerateViewIconTasks(private val activity: MarkerPlugin, private val refreshSource: Boolean = true) {
        ...
        private fun backgroundProcess(vararg params: Feature): AbstractMap<String, Bitmap>? {
            ...
            val bubbleLayout = inflater.inflate(R.layout.symbol_layer_info_window_layout, null) as BubbleLayout
            if (feature.hasProperty(PROPERTY_NAME)) {
    ...
    override fun onMapClick(latLng: LatLng): Boolean {
        ...
        val features = mapplsMap.queryRenderedFeatures(this.mapplsMap.projection.toScreenLocation(latLng), LAYER_ID)
