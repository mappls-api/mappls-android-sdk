...
class ClusterMarkerPlugin(private val mapView: MapView, private val mapplsMap: MapplsMap) :
    ...
    private var featureCollection = Utils.EMPTY_FEATURE
    init {
    ...
    private fun initialiseClusterLayer(style: Style) {
        val layers = arrayOf(
            intArrayOf(10, "#339E82".toColorInt()),
            intArrayOf(5, "#339E82".toColorInt()),
            intArrayOf(0, "#339E82".toColorInt())
        )
...
// Add a filter to the cluster layer that hides the circles based on "point_count"
    ...
    private fun initImages(style: Style) {
        if (style.getImage("marker-icon") == null) {
            style.addImage(
                "marker-icon",
                ContextCompat.getDrawable(mapView.context, R.drawable.mappls_map_demo_marker)!!
            )
        }
    }
    ...
    private fun initialiseSource(style: Style) {
        ...
        style.addSource(
            GeoJsonSource(
                ...
                GeoJsonOptions().withCluster(true).withClusterRadius(50).withClusterMaxZoom(16)
                    .withClusterProperty("has", Expression.has(MARKER_CLUSTER_PROPERTY), Expression.has(MARKER_CLUSTER_PROPERTY))
            )
        )
    }
