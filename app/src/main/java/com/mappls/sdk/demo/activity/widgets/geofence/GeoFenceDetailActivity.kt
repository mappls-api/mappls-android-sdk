...
import com.mappls.sdk.demo.adapter.GeoFenceDetailAdapter
...
import com.mappls.sdk.maps.camera.CameraUpdateFactory.newLatLngBounds
import com.mappls.sdk.maps.camera.CameraUpdateFactory.newLatLngZoom
...
class GeoFenceDetailActivity: AppCompatActivity(), OnMapReadyCallback {
    ...
    private var geofenceDetails: ArrayList<GeofenceDetailModel> = ArrayList()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        mBinding = ActivityGeoFenceDetailBinding.inflate(layoutInflater)
        ...
        mBinding.geoFenceDetailsHeader.headerTitle.text = "GeoFence Detail"
        mBinding.geoFenceDetailsHeader.headerBack.setOnClickListener {
            finish()
        }
        ...
        geoFenceDetailAdapter.setOnGeoFenceChangeListener(object : GeoFenceDetailAdapter.OnGeoFenceChangeListener {
            ...
            override fun onEditGeoFence(geofenceDetail: GeofenceDetailModel) {
                val intent =
            ...
            override fun onRemoveGeofence(geofenceDetail: GeofenceDetailModel) {
    ...
    private fun setBounds(latLngBound: List<LatLng>) {
        ...
        if (latLngBound.size == 1) {
            mapplsMap?.animateCamera(CameraUpdateFactory.newLatLngZoom(latLngBound[0], 16.0))
        } else {
    ...
    private fun removeGeofence(geofenceDetail: GeofenceDetailModel) {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 101) {
            if (resultCode == Activity.RESULT_OK) {
                mapplsMap?.getStyle {
                    val newGeofenceDetail = Gson().fromJson(data?.getStringExtra("GEOFENCE"), GeofenceDetailModel::class.java)
                    mBinding.rootBottomSheet.visibility = View.VISIBLE
                    if (newGeofenceDetail.gfLabel == null) {
                        ...
                        geoFenceDetailAdapter.setGeofenceDetailList(listOf(newGeofenceDetail))
                        if (fillManager == null) {
                            fillManager = FillManager(mBinding.mapView, mapplsMap!!, it)
                        }
                        if (lineManager == null) {
                            lineManager = LineManager(mBinding.mapView, mapplsMap!!, it)
                        }
    ...
    private fun updateGeoFence(geofenceDetail: GeofenceDetailModel) {
        ...
        val polygon: Polygon
        polygon = if (geofenceDetail.gfType.equals(GeofenceDetailModel.TYPE_POLYGON, ignoreCase = true)) {
    ...
    private fun addIcons(geofenceDetail: GeofenceDetailModel) {
        ...
        if (geofenceDetail.gfType.equals(GeofenceDetailModel.TYPE_POLYGON, ignoreCase = true)) {
            ...
            val symbolOptions = SymbolOptions().position(geofenceDetail.circleCentre)
                .icon(BitmapUtils.getBitmapFromDrawable(ContextCompat.getDrawable(this, R.drawable.map_marker_icon)))
                .data(jsonObject)
    ...
    private fun updateGeoFenceData(geofenceDetail: GeofenceDetailModel, newGeofenceDetail: GeofenceDetailModel) {
    ...
    private fun addGeoFence(geofenceDetail: GeofenceDetailModel) {
        val polygon: Polygon
        val jsonObject = JsonObject()
        jsonObject.addProperty("GeoFencetype", geofenceDetail.gfLabel)
        if (geofenceDetail.gfType.equals(GeofenceDetailModel.TYPE_POLYGON, ignoreCase = true)) {
        ...
        if (geofenceDetail.gfType.equals(GeofenceDetailModel.TYPE_CIRCLE, ignoreCase = true)) {
            points.add(points[0])
        }
