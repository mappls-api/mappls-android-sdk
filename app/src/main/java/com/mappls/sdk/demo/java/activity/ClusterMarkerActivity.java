...
public class ClusterMarkerActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(@NonNull MapplsMap mapplsMap) {
        mapplsMap.animateCamera(CameraUpdateFactory.newLatLngBounds(new LatLngBounds.Builder().includes(latLngList).build(), 10, 10, 10, 10));
        ClusterMarkerPlugin plugin = new ClusterMarkerPlugin(mBinding.mapView, mapplsMap);
