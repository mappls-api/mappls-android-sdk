...
#### Kotlin
```kotlin
val mapplsGeolocation = MapplsGeolocation.builder()
    .addCellTower(cellId, lac, mcc, mnc)
    .build()
//To call In Foreground Thread
MapplsGeolocationManager.newInstance(mapplsGeolocation).call(object : OnResponseCallback<GeolocationResponse?> {
    override fun onSuccess(geolocationResponse: GeolocationResponse?) {
    }
...
val geolocationResponse = MapplsGeolocationManager.newInstance(mapplsGeolocation).executeCall()
```
#### Java
```java
