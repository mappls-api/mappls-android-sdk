...
3. `pod` (string) = it takes in the place type code which helps in restricting the results to certain chosen type.**Below mentioned are the codes for the pod**
...
4. `filter`(string)  = This parameter helps you restrict the result either by mentioning a bounded area or to certain mapplspin (6 digit code to any poi, locality, city, etc.) below mentioned are the various types:
...
5. `index`(Integer): the order where this result should be placed
6. `distance`(Double): aerial distance in meters from reference location
...
6. `pod (string)` : it takes in the place type code which helps in restricting the results to certain chosen type. Access to this parameter is controlled from the backend. This parameter if provided will override any values provided in `keywords` request param.
   **Below mentioned are the codes for the pod**
	-   NearbyCriteria.POD_SUB_LOCALITY
	-   NearbyCriteria.POD_LOCALITY
	-   NearbyCriteria.POD_CITY
	-   NearbyCriteria.POD_VILLAGE
7.  `filter(String)` : This parameter helps you get a specific type of EV charging Station
...
5. ``filter(String)``: This feature in POI Along the Route API empowers the user a fine discovery of EV charging stations along with the existing keyword and category code lookups. It uses multiple keys like "model", "plugType", "macVType". It uses key:value pair(s).
(e.g. plugType:IEC)
>>  - Each key:value pair is seperated by a seperator, (e.g. filter= plugType:IEC; model:Mahindra Electric).
>>  - User can use two or more values for a key by putting either OR(;) or AND($) operator (e.g. plugType:IEC&PN3; model:Mahindra Electric).
...
17. `hourOfOperation`(String): To allow access open_close_hours.
18. `longDescription`(String):
19. `shortDescription`(String): 
20. `richInfo`(Map): 
21. `partnersFlag`(List<`PartnerFlag`>)

#### PartnerFlag Response parameters
1. `richInformationFlag`(Boolean):
2. `categoryName`(String):
3. `categoryIcon`(String):
4. `previewText`(String):
5. `providerId`(String):
