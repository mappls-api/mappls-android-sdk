...
#### Kotlin
```kotlin
val nearbyReport = MapplsNearbyReport.builder()
...
MapplsNearbyReportManager.newInstance(nearbyReport).call(object : OnResponseCallback<NearbyReportResponse?> {
...
val response = MapplsNearbyReportManager.newInstance(nearbyReport).executeCall()
```
#### Java
```java
